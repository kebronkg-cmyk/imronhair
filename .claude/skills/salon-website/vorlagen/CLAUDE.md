...
Übernommen aus dem JaSuVi-Projekt und im Irmonhair-Projekt erweitert —
Konventionen, Vorlieben des Auftraggebers und Fallen, die tatsächlich
zugeschlagen haben. Die übertragbare Norm (Ablauf, Prüfskripte, Ornamente
aus Vorlagen) steht im Skill `.claude/skills/salon-website/`; für ein neues
Salon-Repo diesen Ordner kopieren und `vorlagen/CLAUDE.md` als `CLAUDE.md`.
...
- **Vorlagen der Inhaberin gewinnen.** Schickt sie Visionsbilder, wird
  deren Welt gebaut — Licht, Material, Schrift, Ornament. Ein Ornament
  daraus (die Schleife) wird im Koordinatenraum des Fotoausschnitts
  nachgezeichnet, mit Volumen und Licht nur dort, wo es im Foto steht,
  und über einen Generator an alle Stellen gesetzt (Auftakt,
  Ladebildschirm, Logo, Favicon). Im Logo nur der erkennbare Ausschnitt.
- **Aussagen aus Visionsbildern sind keine Belege.** Zahlen, „Premium“,
  „schonend“ erst nach Bestätigung; bis dahin weglassen und fragen.
- **Eine Lampe pro Knopfgruppe.** Nur der Hauptknopf leuchtet; leuchten
  zwei, führt keiner.
...
2. `node .claude/skills/impeccable/scripts/detect.mjs --json` muss `[]` liefern —
   oder nur Meldungen, die in `UEBERGABE.md` einzeln als gewollt begründet sind.
...
| `#` in einer SVG-Daten-URI (Maske, Muster) — das Bild bleibt still leer | `%23` schreiben |
| Prüfaufnahmen mitten im Auftritt; der Prüfer verwirft sie | `auftritt-da` setzen, 1,2 s warten, letzte Kachel an `scrollHeight − h` (`salon-website/scripts/aufnahme.mjs`) |
| Sobald `DESIGN.md` existiert, meldet der Detektor jede Rohfarbe und jeden Zwischengrad | Rohwerte als Token, Alpha über `color-mix(in oklch, var(--x) 35%, transparent)`, Schrifttreppe als `typography.scale` |
| `box-shadow: inset 0 0 0 1px` gilt dem Detektor als Glühen | `border` nehmen |
| CSS-`stroke` überschreibt das `stroke`-Attribut im SVG | Farbe am Element, nur Breite und Animation in CSS |
